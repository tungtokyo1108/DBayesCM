...
class EncoderDBGCM(nn.Module):
    ...
    def __init__(
        ...
        if self.use_LayerNorm:
            ...
            self.input_norm_miRNA = nn.LayerNorm(n_input_miRNA)
        ...
        self.encoder_miRNA = MaskedLinearLayers(
                    n_in=n_input_miRNA,
                    n_out=n_hidden,
                    n_cat_list=n_cat_list,
                    mask=mask,
        ...
        if self.combine_method == 'concat':
            dim_encoder_shared = n_hidden + n_hidden + n_hidden
        ...
        nn.init.xavier_uniform_(self.z_var_encoder.weight, gain=0.01)
    ...
    def reparameterize_transformation(self, mu, var):
        ...
        var = torch.clamp(var, min=1e-5, max=1e5)
        std = torch.sqrt(var)
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, y_1: torch.Tensor, *cat_list: int):
        
        # Normalize input
        if self.use_LayerNorm:
            x = self.input_norm_microbiome(x)
            y = self.input_norm_metabolite(y)
            y_1 = self.input_norm_miRNA(y_1)
        
        q_x = self.encoder_microbiome(x, *cat_list)
        q_y = self.encoder_metabolite(y, *cat_list)
        q_y1 = self.encoder_miRNA(y_1, *cat_list)
        
        if self.combine_method == 'concat':
            q = torch.cat([q_x, q_y, q_y1], dim=-1)
        elif self.combine_method == 'add':
            q = (q_x + q_y + q_y1)/3.0
        else:
            raise ValueError("combine method must choose from concat or add")  
        ...
        # Get latent parameters with improved stability
        qz_m = self.z_mean_encoder(q)
        #qz_v = torch.exp(self.z_var_encoder(q)) + 1e-4
        #qz_v = torch.exp(torch.clamp(self.z_var_encoder(q), -4.0, 4.0)/2.)
        
        qz_v = torch.exp(torch.clamp(self.z_var_encoder(q), -10.0, 10.0))
        
        # Add small epsilon to variance for numerical stability
        qz_v = qz_v + 1e-6
        
        latent = reparameterize_gaussian(qz_m, qz_v)
...
class DecoderDBGCM_vMF(nn.Module):
    ...
    def __init__(
        ...
        self.n_output_miRNA = n_output_miRNA
        ...
        # Variational parameters of SS-GHS for miRNA
        # Prior parameters
        self.logit_0_mr = nn.Parameter(torch.logit(torch.ones(1) * pip0, eps=1e-6), requires_grad=False)
        self.bias_d_mr = nn.Parameter(torch.zeros(1, self.n_output_miRNA))
        
        # Weight parameters
        self.slab_mean_mr = nn.Parameter(torch.randn(self.n_clusters, self.n_output_miRNA) * 0.1)
        self.slab_lnvar_mr = nn.Parameter(torch.ones(self.n_clusters, self.n_output_miRNA) * (-1.0))
        
        # Node inclusion probability logits (for variational distribution)
        self.spike_logit_mr = nn.Parameter(torch.zeros(self.n_clusters, self.n_output_miRNA) * self.logit_0_mr)
        
        # Local horseshoe parameters (per node)
        # Each cluster has its own horseshoe local parameters (τ² = βα)
        self.alpha_mu_mr = nn.Parameter(torch.zeros(self.n_clusters, self.n_output_miRNA))
        self.alpha_log_var_mr = nn.Parameter(torch.ones(self.n_clusters, self.n_output_miRNA) * (-6.0))
        self.beta_mu_mr = nn.Parameter(torch.zeros(self.n_clusters, self.n_output_miRNA))
        self.beta_log_var_mr = nn.Parameter(torch.ones(self.n_clusters, self.n_output_miRNA) * (-6.0))
    ...
    def forward(self, z: torch.Tensor,):
        ...
        # miRNA
        rho_mr = self.get_beta(batch_size, self.n_output_miRNA, self.bias_d_mr, self.spike_logit_mr, self.slab_lnvar_mr, self.slab_mean_mr, 
                            self.alpha_mu_mr, self.alpha_log_var_mr, self.beta_mu_mr, self.beta_log_var_mr, 
                            self.zeta_a_mu, self.zeta_a_log_var, self.zeta_b_mu, self.zeta_b_log_var)
        rho_kl_mr = self.sparse_kl_loss(self.spike_logit_mr, self.slab_lnvar_mr, self.slab_mean_mr, 
                            self.alpha_mu_mr, self.alpha_log_var_mr, self.beta_mu_mr, self.beta_log_var_mr, 
                            self.zeta_a_mu, self.zeta_a_log_var, self.zeta_b_mu, self.zeta_b_log_var)
        
        return rho, rho_kl, rho_me, rho_kl_me, rho_mr, rho_kl_mr, log_theta_dp, theta_dp, theta_dp_kl
...
class DBayesCM(pl.LightningModule):
    ...
    def __init__(
        ...
        self.decoder = DecoderDBGCM_vMF(
            ...
            n_output_miRNA=self.n_input_mr,
            pip0=self.pip0_rho,
        ...
        # miRNA
        self.val_rho_mr_all_epochs = []
        self.val_z_sample_mr_all_epochs = []
        self.train_rho_mr_all_epochs = []     
        self.train_z_sample_mr_all_epochs = []
    ...
    def get_decoder_outputs(self, x: torch.Tensor, y: torch.Tensor, y1: torch.Tensor):
        """
        ...
        qz_m, qz_v, z = self.z_encoder(x_,y_,y_1)
        # vMF decoder normalizes z internally
        rho, rho_kl, rho_me, rho_kl_me, rho_mr, rho_kl_mr, log_theta_dp, theta_dp, theta_dp_kl = self.decoder(z)
        ...
        if rho_mr.dim() == 3:
            rho_mr = rho_mr.mean(dim=0)
            
        return rho, rho_me, rho_mr, theta_dp
    ...
    def compute_loss(self, x, y, y1, kl_weight=1.0):
        ...
        y_1 = torch.clamp(y_1, min=0.0)

        # 2) Forward pass through encoder
        #torch.nn.utils.clip_grad_norm_(self.z_encoder.parameters(), max_norm=1.0)
        encoder_input_microbiome = x_
        encoder_input_metabolite = y_
        encoder_input_miRNA = y_1
        qz_m, qz_v, z = self.z_encoder(encoder_input_microbiome, encoder_input_metabolite, encoder_input_miRNA)
        ...
        #    Dirichlet multinominal for miRNA
        if rho_mr.dim() == 3:
            # For each sample in batch, compute mean log_aa across the batch dimension
            batch_size = rho_mr.size(0)
            log_aa_list = []
            
            for i in range(batch_size):
                curr_theta = theta_dp[i:i+1]  # Keep dimension [1, n_clusters]
                curr_rho = rho_mr[i]  # [n_clusters, n_output]
                curr_log_aa = torch.clamp(torch.mm(curr_theta, curr_rho), -10, 10)
                log_aa_list.append(curr_log_aa)
                
            log_aa_mr = torch.cat(log_aa_list, dim=0)
        else:
            # Original case for 2D rho
            log_aa_mr = torch.clamp(torch.mm(theta_dp, rho_mr), -10, 10)
            
        aa_mr = torch.exp(log_aa_mr)
        reconstruction_loss_mr = -self.dir_llik(y_1, aa_mr)
        ...
        kl_divergence_beta_mr = rho_kl_mr
        ...
        loss = (torch.mean(reconstruction_loss + reconstruction_loss_me + reconstruction_loss_mr + kl_weight * kl_local) 
                + kl_weight_beta * kl_divergence_beta/(10.0 * x.shape[1])
                + kl_weight_beta * kl_divergence_beta_me/y.shape[1]
                + kl_weight_beta * kl_divergence_beta_mr/y_1.shape[1]
    ...
    def on_train_epoch_start(self):
        ...
        # miRNA
        self._train_rho_mr_list = []
        self._train_z_sample_mr_list = []
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            rho, rho_me, rho_mr, theta = self.get_decoder_outputs(x, y, y1)
            z_sample = torch.sigmoid(self.decoder.spike_logit)
            z_sample_me = torch.sigmoid(self.decoder.spike_logit_me)
            z_sample_mr = torch.sigmoid(self.decoder.spike_logit_mr)
        ...
        # miRNA
        self._train_rho_mr_list.append(rho_mr.detach().cpu())
        self._train_z_sample_mr_list.append(z_sample_mr.detach().cpu())
    ...
    def on_train_epoch_end(self):
        ...
        if len(self._train_rho_list) > 0:
            ...
            all_rho_mr = torch.cat(self._train_rho_mr_list, dim=0)
            all_z_sample_mr = torch.stack(self._train_z_sample_mr_list, dim=0)
            ...
            self.train_rho_mr_all_epochs.append(all_rho_mr)
            self.train_z_sample_mr_all_epochs.append(all_z_sample_mr)
    ...
    def on_validation_epoch_start(self):
        ...
        self._val_rho_mr_list = []
        self._val_z_sample_mr_list = []
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self._val_rho_mr_list.append(rho_mr.detach().cpu())
        self._val_z_sample_mr_list.append(z_sample_mr.detach().cpu())
    ...
    def on_validation_epoch_end(self):
        ...
        if len(self._val_rho_list) > 0:
            ...
            all_rho_mr = torch.cat(self._val_rho_mr_list, dim=0)
            all_z_sample_mr = torch.stack(self._val_z_sample_mr_list, dim=0)
            ...
            self.val_rho_mr_all_epochs.append(all_rho_mr)
            self.val_z_sample_mr_all_epochs.append(all_z_sample_mr)
    ...
    def collect_decoder_outputs_for_dataset(self, data_loader):
        ...
        with torch.no_grad():
            for batch in data_loader:
                ...
                all_rho_mr.append(rho_mr.cpu().unsqueeze(0))
...
class MicrobiomeDataset(Dataset):
    def __init__(self, X, Y, Y1, ground_true):
        super().__init__()
